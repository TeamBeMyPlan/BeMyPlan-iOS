...
protocol PurchaseServiceType{
  ...
  func purchaseTravelPlan(planIdx: Int,completion: @escaping (Result<PurchaseCheckModel?, Error>) -> Void)
  func validateReceipt(orderID: Int,receipt:String,completion: @escaping (Result<PurchaseValidateModel?, Error>) -> Void)
  func confirmTravelPurchase(orderID: Int,paymentId: Int, userID: Int,completion: @escaping (Result<Bool?, Error>) -> Void)
}
...
extension BaseService : PurchaseServiceType {
  ...
  func purchaseTravelPlan(planIdx: Int, completion: @escaping (Result<PurchaseCheckModel?, Error>) -> Void) {
    requestObject(.purchaseTravelPlan(idx: planIdx), completion: completion)
  }
